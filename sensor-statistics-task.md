...
Create a command line program that calculates statistics from humidity sensorMeasurement data.

### Background story

The sensors are in a network, and they are divided into groups. Each sensorMeasurement submits its data to its group leader.
...
- Measurement line has sensorMeasurement id and the humidity value
- Humidity value is integer in range `[0, 100]` or `NaN` (failed measurement)
- The measurements for the same sensorMeasurement id can be in the different files
...
- For each sensorMeasurement it calculates min/avg/max humidity
...
sensorMeasurement-id,min,avg,max
s2,78,82,88
s1,10,54,98
s3,NaN,NaN,NaN
```
